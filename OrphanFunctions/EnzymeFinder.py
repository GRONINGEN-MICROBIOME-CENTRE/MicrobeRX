...
class GetUniprotData:
    def __init__(self, query: str, query_type: str):
        ...
        species_chunks = list(
            __generate_chunks(self.organisms_database.NCBI_taxon_ID.unique(), 200)
        )

        data = [
            f'({query_type}:{query})+AND+taxonomy_id:({"+OR+".join(map(str,d))})'
            for d in species_chunks
        ]
        ...
        self.urls = [
            f"https://rest.uniprot.org/uniprotkb/search?fields={','.join(self.fields.keys())}&format=tsv&query={u}&size=500"
            for u in data
        ]
    ...
    def run_search(self):
        ...
        self.uniprot_data = pd.concat(
            [pd.read_csv(StringIO(result), sep="\t") for result in results]
        )
        self.uniprot_data.drop_duplicates(inplace=True, ignore_index=True)
        self.uniprot_data.columns = self.fields.values()
        self.uniprot_data.sort_values(
