def PlotEvidencesDistribution(table: pd.DataFrame, outfile: str = None):
    ...
    group = nT.groupby(['METABOLITE_id','reaction_EC']).size().reset_index(name="Frequency")
    group["Frequency"]=[1 for i in group.Frequency]
    
    # count total number of evidence codes
    group['EC_count']=group.reaction_EC.map(group.groupby('reaction_EC')['Frequency'].count().to_dict())
    
    # sort the dataframe by EC_count
    group.sort_values('EC_count',ascending=False,inplace=True,ignore_index=True)
    
    # plot the graph
    fig = px.bar(data_frame=group, y='Frequency',x='reaction_EC',color='METABOLITE_id',orientation='v',
                 pattern_shape='METABOLITE_id', color_discrete_sequence=px.colors.qualitative.Pastel,
                 pattern_shape_sequence=['', '/', '\\', 'x','', '-', '|', '+', '.','']).update_xaxes(categoryorder='total descending')
    
    # update graph layout
    fig.update_layout(title_text="Evidences Per Result",height=600, width=1200,plot_bgcolor='rgba(0, 0, 0, 0)',font=dict(size=12))
    if outfile:
...
def PlotMetaboliteInfo(singleMetTable:pd.DataFrame, outfile: str = None):
    ...
    mz={float(value.split(':')[0]):float(value.split(':')[1]) for value in singleMetTable.MZ_distribution.iloc[0].split(';')}

    F = make_subplots(rows=1, cols=2,specs=[[{"type": "table"},{"type": "bar"}]],subplot_titles=("Result information","Isotopic distribution"))

    F.add_trace(go.Bar(name='Metabolite', x=tuple(mz.keys()), y=tuple(mz.values())),row=1, col=2)
    ...
    F.update_layout(width=1200,height=600,plot_bgcolor='rgba(0, 0, 0, 0)',font=dict(size=12),xaxis_title="Atomic Mass (u)",yaxis_title="Relative abundance (%)")
    if outfile:
        F.write_image(outfile,height=400*rows, width=400*cols,validate=True,scale=2)
    else:
        pass
    return F
...
def PlotClusterMap(singleMetabolite:pd.DataFrame,outfile: str = None, selection:str='all',n_samples:int=75):
    ...
    print(f'{len(sample.index)} unique sequences found for {len(sample.ORGANISM_name.unique())} species')
    if len(sample.index)>=n_samples:
        print(f'Sampling ~{n_samples}sequences with higher existence and species diversity')
        max_quality=sample[sample.EXISTENCE=='Evidence at protein level']
        max_quality=max_quality.groupby(['EVIDENCE','ORGANISM_name']).apply(lambda g: g.sample(int(1 * len(g)/len(sample)+1)))
        if len(max_quality)<n_samples:
            sample=sample[sample.EXISTENCE!='Evidence at protein level']
            sample = sample.groupby(['EVIDENCE']).apply(lambda g: g.sample(int((n_samples-len(max_quality)) * len(g)/len(sample)+1)))
    ...
    for index in sample.index:
        for jndex in sample.index:
            ...
            hmap.loc[f'{sample.ENTRY_id[index]} : {sample.ORGANISM_name[index].split("(")[0]}',sample.ENTRY_id[jndex]]=identity
    ...
    ax=sns.clustermap(hmap.reset_index(drop=True),figsize=(17,15),xticklabels=1,yticklabels=hmap.index,cmap='RdBu_r',method='complete',vmin=0,vmax=100,tree_kws=dict(linewidths=1.5),
                      row_colors=[row_colors,row_colors2],col_colors=[col_colors])
    ...
    for label in sample.SUPERKINGDOM.unique():
        ax.ax_row_dendrogram.bar(0, 0, color=lut_phylum[label], label=label, linewidth=0);
    l1=ax.ax_row_dendrogram.legend(title='Taxonomy', loc="center", ncol=1, bbox_to_anchor=(0.05, 0.03),bbox_transform=plt.gcf().transFigure,frameon=False)

    for label in sample.EXISTENCE.unique():
        ax.ax_col_dendrogram.bar(0, 0, color=lut_ex[label], label=label, linewidth=0);
    l2=ax.ax_col_dendrogram.legend(title='Existence', loc="center", ncol=1, bbox_to_anchor=(0.87, 0.82),bbox_transform=plt.gcf().transFigure,frameon=False)


    xx = []
    for label in sample.EVIDENCE.unique():
        x = ax.ax_row_dendrogram.bar(0, 0, color=lut_ev[label], label=label, linewidth=0)
        xx.append(x)

    l3 = plt.legend(xx, sample.EVIDENCE.unique(), loc="center", title='Evidence', bbox_to_anchor=(0.15, 0.03), bbox_transform=plt.gcf().transFigure,frameon=False)

    if isinstance(outfile,str):
        ax.savefig(outfile,dpi=300,format='svg',bbox_inches='tight')
    return sample,ax

def PlotSequenceLogo(table:pd.DataFrame, prefix:str='MSA', selection:str='all', outfile: str = None)->None:
    """
    ...
    domsTable=table.dropna(subset=['DOMAIN_location'])[['ENTRY_id','DOMAIN_location','LENGHT']].sort_values(by='LENGHT',ascending=True).reset_index(drop=True)
    ...
    for index in domsTable.index:
        ...
        for didx,name in enumerate(namesDomains):
            domains_dict[name]=list(map(int,domain_pattern.findall(domsTable.DOMAIN_location[index])[didx].split('..')))
...
def PlotGO(table: pd.DataFrame, outfile: str = None, selection: str = 'all'):
    ...
    go_process=go_table.groupby(['GO_process','ORGANISM_name'])['SEQUENCE'].count().reset_index(['GO_process','ORGANISM_name'])

    go_process.sort_values(by='ORGANISM_name',inplace=True,ignore_index=True)

    fig1 = px.bar(data_frame=go_process, x='SEQUENCE',y='GO_process', color='ORGANISM_name',
    ...
    go_function=go_table.groupby(['GO_function','ORGANISM_name'],dropna=False)['SEQUENCE'].count().reset_index(['GO_function','ORGANISM_name'])

    go_function.sort_values(by='ORGANISM_name',inplace=True,ignore_index=True)

    fig2 = px.bar(data_frame=go_function, x='SEQUENCE',y='GO_function', color='ORGANISM_name',
                 orientation='h',color_discrete_sequence=px.colors.qualitative.Pastel)
