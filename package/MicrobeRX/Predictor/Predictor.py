...
class MetabolitePredictor:
    ...
    def run_prediction(self, query:Chem.Mol,name:str='metabolite'):
        ...
        for index in tqdm(self.rules_table.index):
            #try:
                Predictor=RunPredictionRule(query=self.query,rule_smarts=self.rules_table.rule[index],real_product=self.rules_table.product_map[index],real_substrate=self.rules_table.substrate_map[index])
                Predictor.predict()
                ...
                if results:
                    ...
                    self.predicted_metabolites=self.predicted_metabolites.append(tmp_table,ignore_index=True)
            #except Exception:
                #pass
        
        self.predicted_metabolites['confidence_score']=round(self.predicted_metabolites.similarity_substrates + self.predicted_metabolites.similarity_products + self.predicted_metabolites.reacting_atoms_efficiency,3)
        self.predicted_metabolites.sort_values(by='confidence_score',ascending=False,ignore_index=True,inplace=True)   
        self.predicted_metabolites.drop_duplicates(subset=['reaction_id','substrate','product','main_product_smiles'],keep='first',inplace=True,ignore_index=True)
        ...
        un_mets=self.predicted_metabolites.drop_duplicates(subset=["main_product_smiles"],keep='first',ignore_index=True)
...
class RunPredictionRule:
    ...
    def predict(self):
        ...
        num_substructure_matches = len(self.query.GetSubstructMatches(self._main_reactant))
        self.unique_products = {}
        if num_substructure_matches > 0:
            
            self._atom_map = [a.GetAtomMapNum() for a in self._main_reactant.GetAtoms() if a.GetAtomMapNum() != 0]
            ...
            for products in results:
                try:
                    ...
                    real_product_atom_indexes= [a.GetIdx() for a in self.real_product.GetAtoms() if a.GetAtomMapNum() in self._atom_map]
                    real_substrate_atom_indexes= [a.GetIdx() for a in self.real_substrate.GetAtoms() if a.GetAtomMapNum() in self._atom_map]
                    
                    query_substructure=self.__get_mol_substructure(self.query, query_atoms_indexes)
                    real_substrate_substructure=self.__get_mol_substructure(self.real_substrate, real_substrate_atom_indexes)    
                    main_product_substructure=self.__get_mol_substructure(main_product, main_product_atom_indexes)
                    real_product_substructure=self.__get_mol_substructure(self.real_product, real_product_atom_indexes)    
                    
                    self.unique_products[smi]={'main_product_smiles':smi,
                                               'secondary_products_smiles':'.'.join([Chem.MolToSmiles(p, canonical=True) for p in products[1:]]),
                                               'similarity_substrates': self.__compute_similarity(self.query,self.real_substrate,fingerPrintModel='rdkit'),
                                               ...
                                               'similarity_products': self.__compute_similarity(main_product,self.real_product,fingerPrintModel='rdkit'),
    ...
    def __get_product_atom_indexes(self,product:Chem.Mol):
        ...
        for atom in product.GetAtoms():
            ...
            if set(['old_mapno', 'react_atom_idx']).issubset(set(atom_properties.keys())):
    ...
    def __get_mol_substructure(self,mol_target:Chem.Mol,atom_indexes_to_keep):
        ...
        """
        atom_indexes_to_keep = self.__get_substructure_neighbors(mol_target,atom_indexes_to_keep)
        atom_indexes_to_remove = [atom.GetIdx() for atom in mol_target.GetAtoms() if atom.GetIdx() not in atom_indexes_to_keep]
    ...
    def __get_substructure_neighbors(self,mol_target:Chem.Mol,atom_indexes_to_keep):
        ...
        for atom_index in atom_indexes_to_keep:
            ...
            neighbor_atoms.extend([neighbor.GetIdx() for neighbor in atom.GetNeighbors()])
            neighbor_atoms.extend([neighbor_neighbor.GetIdx() for neighbor in atom.GetNeighbors() for neighbor_neighbor in neighbor.GetNeighbors()])
            neighbor_atoms.extend([a for ring in rings.AtomRings() for a in ring if atom_index in ring])  
    ...
    def __compute_similarity(self,mol1:Chem.Mol, mol2:Chem.Mol,fingerPrintModel:str='maccskey'):
        ...
        if fingerPrintModel == 'maccskey':
            similarity=DataStructs.FingerprintSimilarity(MACCSkeys.GenMACCSKeys(mol1),MACCSkeys.GenMACCSKeys(mol2))
